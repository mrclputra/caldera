...
void read_ply_file(const std::string &filepath, const bool preload_into_memory = true) {
   ...
   try {
      ...
      if (tristrip)
         std::cout << "\tRead " << (tristrip->buffer.size_bytes() / tinyply::PropertyTable[tristrip->t].stride) << " total indices (tristrip) " << std::endl;
      if (faces->list_sizes.size())
         std::cout << "\tRead " << faces->list_sizes.size() << " varible-length indices " << std::endl;
